...
def get_tiny_imagenet_loaders(num_clients, batch_size, data_dir='./data/tiny-imagenet-200'):
    transform_train = transforms.Compose([
    ...
    # Restructure val directory if not already done
    ...
    print(f"Dataset: Tiny ImageNet | Images/Client: {split_size}")
    return client_loaders, test_loader
...
def train_standard_split(client_loaders, test_loader, num_splits, tracker):
    ...
    model = Net(depth=110, num_classes=200, num_splits=num_splits).to(DEVICE)
